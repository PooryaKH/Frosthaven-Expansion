...
function modifier_baby_broodmother_passive:OnCreated( kv )
	if IsServer() then
		...
		if GameRules.holdOut.hFort then
			self:GetParent():SetInitialGoalEntity( GameRules.holdOut.hFort )
		else
			local hFort = GameRules.holdOut.FindAndGetFort()
			if hFort then
				self:GetParent():SetInitialGoalEntity( hFort )
			else
				print( "ERROR - modifier_baby_broodmother_passive: Could not set initial goal entity" )
				self:Destroy()
				return
			end
		end
		...
		if #hEnemies > 0 then
			for _, hEnemy in pairs( hEnemies ) do
				if hEnemy ~= nil and ( not hEnemy:IsMagicImmune() ) and ( not hEnemy:IsInvulnerable() ) then
					...
					hEnemy:AddNewModifier( self:GetParent(), self:GetAbility(), "modifier_knockback", kv )
					]]
				end
			end
		end
